...
Deno.test("the OpenAI-compatible fetch adapter streams through the WASM tool loop", async () => {
      ...
      const records =
        requestCount === 1
                      ...
                      tool_calls: [
                        {
                          index: 0,
                          id: "add-1",
                          function: { name: "add", arguments: '{"left":2,"right":' },
                        },
                      ],
                    },
                  },
          ...
          : [
              ...
              {
                choices: [{ delta: { content: "6" }, finish_reason: "stop" }],
                usage: {
                  prompt_tokens: 5,
  ...
  try {
    ...
    assert(
      secondMessages.some((message) => message.role === "tool" && message.content === "6"),
      "tool output should use OpenAI's tool role",
    )
    agent.close()
  } finally {
