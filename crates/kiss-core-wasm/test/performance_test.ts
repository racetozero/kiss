...
function report(name: string, samplesMs: number[], iterations: number, work: string): void {
  const meanMs = samplesMs.reduce((total, sample) => total + sample, 0) / samplesMs.length
...
Deno.test("warm turns and 25 isolated agents stay within browser budgets", async () => {
  ...
  assert(promptP50 < 5, `warm prompt p50 exceeded 5ms: ${promptP50.toFixed(3)}ms`)
  assert(promptP95 < 20, `warm prompt p95 exceeded 20ms: ${promptP95.toFixed(3)}ms`)
  agent.close()
  ...
  assert(capacityP95 < 1_000, `25-agent fixture exceeded 1s p95: ${capacityP95.toFixed(3)}ms`)

  report("wasm_module_init", [initMs], 1, "cached_deno_module")
  report("wasm_warm_prompt", promptSamples, 1, "host_model_full_agent_turn")
  report("wasm_parallel_agents_25", capacitySamples, 1, "25_isolated_full_agent_turns")
})
