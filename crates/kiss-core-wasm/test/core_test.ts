...
Deno.test("the agent and host-tool loop run entirely inside WASM", async () => {
  ...
  const secondContext = (requests[1].context as { messages: Array<Record<string, unknown>> })
    .messages
  assert(
    secondContext.some((message) => message.role === "toolResult"),
    "tool output must enter model context",
  )
  ...
  assert(
    agent.state().messageCount === result.messages.length,
    "history should be retained in WASM",
  )
  ...
  const restoredContext = restoredRequests[0].context as {
    messages: Array<Record<string, unknown>>
  }
  assert(
    restoredContext.messages.length > 1,
    "checkpoint history should reach the next model request",
  )
...
Deno.test("steering and idle controls are owned by the WASM session", async () => {
  ...
  const agent = KissAgent.create(
    { model },
    async (request: { context: { messages: Array<Record<string, unknown>> } }) => {
      calls += 1
  ...
  assert(
    contexts[1].messages.some(
      (message) => message.role === "user" && message.content === "new priority",
    ),
    "the steered message should enter the second model context",
  )
  assert(
    contexts[2].messages.some(
      (message) => message.role === "user" && message.content === "after steering",
    ),
  ...
  assert(
    ...
    "unsupported reasoning must be clamped by the model",
  )
...
Deno.test("schema failures stay in the agent loop and cancellation reaches the host", async () => {
  ...
  let entered = false
  let abortCalls = 0
  const abortAgent = KissAgent.create(
    { model },
    (_request: unknown, _emit: unknown, signal: AbortSignal) => {
      abortCalls += 1
