    ...
    for (const partial of delta.tool_calls ?? []) {
      ...
      if (partial.function?.arguments) {
        ...
        emit({
          type: "toolcall_delta",
          contentIndex: call.contentIndex,
          delta: partial.function.arguments,
        })
      }
    }
  }
...
function normalizeUsage(usage = {}) {
  ...
  return {
    ...
    cacheWrite,
    cacheReadAvailable:
      details != null && ("cached_tokens" in details || "cache_write_tokens" in details),
...
function mapStopReason(reason, content) {
  ...
  if (reason === "tool_calls" || content.some((block) => block.type === "toolCall"))
    return "toolUse"
  return "stop"
}
